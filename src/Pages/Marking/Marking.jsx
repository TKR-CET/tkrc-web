  ...
  useEffect(() => {
    const fetchExistingData = async () => {
      try {
        ...
        if (data && data.periods && data.attendance) {
          // Set existing attendance and form data if available
          setExistingData(data.periods || []);
          setAttendance(
            data.attendance.reduce((acc, student) => {
          ...
          setEditable(true);  // Mark the form as editable
        }
      } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch("https://tkrcet-backend.onrender.com/attendance/mark-attendance", {
        method: editable ? "PUT" : "POST", // Use PUT for editing, POST for new data
        headers: {
