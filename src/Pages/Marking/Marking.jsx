...
const Marking = () => {
  ...
  // Fetch periods for the subject
  useEffect(() => {
    if (!facultyId || !department || !section || !subject) return;

    const fetchPeriodsForSubject = async () => {
      try {
        const response = await fetch(
          `https://tkrcet-backend-g3zu.onrender.com/faculty/${facultyId}/${department}/${section}/${subject}`
        );
